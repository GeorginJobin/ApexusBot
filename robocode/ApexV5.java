...
/**
 * ApexV5 - Best of Both Worlds
 * 
 * PROVEN COMPONENTS:
 * - V3's working radar lock system
 * - V4's aggressive fire control
 * - V4's improved bullet power
 * - V3's smooth movement
 * 
 * IMPROVEMENTS:
 * - Tighter radar lock (5° instead of 10°)
 * - Faster gun tracking
 * - Higher DPS
 */
public class ApexV5 extends Robot {
    ...
    /**
     * Main run method - V3's PROVEN radar system
     */
    public void run() {
        // Set colors
        ...
        // Main loop - SIMPLE and WORKING
        while (true) {
            if (currentTarget == null) {
                // Wide sweep to find enemies
                turnRadarRight(45);
            } else {
                // Continuous sweep when tracking
                // The actual lock happens in onScannedRobot
                turnRadarRight(36); // Faster sweep than V3's 45°
            }
        }
    }
    ...
    private void fireControlSystem(EnemyData enemy) {
        ...
        if (getGunHeat() == 0 && enemy.energy > 0 && getEnergy() > firePower) {
            ...
            if (Math.abs(gunTurn) < 15) { // Even more aggressive than V4's 20°
                fire(firePower);
            }
        }
    }
