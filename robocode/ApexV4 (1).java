    ...
    public void run() {
        ...
        while (true) {
            if (currentTarget == null) {
                ...
                // INFINITY LOCK: Calculate angle to enemy
                double radarTurn = normalizeBearing(
                    Math.toDegrees(enemyAbsoluteBearing) - getRadarHeading()
                );
                
                // Add overshoot to maintain lock (degrees not radians!)
                if (radarTurn < 0) {
                    radarTurn -= 5; // Overshoot left
                } else {
                    radarTurn += 5; // Overshoot right
                }
