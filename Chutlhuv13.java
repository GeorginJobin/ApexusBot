...
public class Chutlhuv13 extends Robot {
    ...
    */
    ...
    public void onScannedRobot(ScannedRobotEvent e) {
        ...
        double turn = normalizeBearing(radarTurn);
		turnRadarRight(turn + (turn >= 0 ? 30 : -30));
    ...
    */
	private void fireControlSystem(EnemyData enemy) {
		...
		if (detectSpinbot()) {
 		   predictedAngle = Math.toRadians(getHeading() + enemy.bearing); // head-on vs spinbot
		} else {
   			 predictedAngle = predictEnemyPosition(enemy, firePower);
		}
	...
	*/
    ...
    // Keep distance band so you don't ram into SpinBot
    if (cur.distance < 180) {
        turnRight(normalRelativeAngle(cur.bearing));
        back(120);
    } else if (cur.distance > 420) {
        turnRight(normalRelativeAngle(cur.bearing));
        ahead(120);
    } else {
        // Orbit / strafe perpendicular
        turnRight(normalRelativeAngle(cur.bearing + 90 * moveDirection));
        ahead(140);
    }

    if (Math.random() < 0.18) moveDirection *= -1;
}
